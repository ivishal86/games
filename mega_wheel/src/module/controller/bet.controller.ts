...
import { BetData, MatchHistoryRow } from '../../interface/headAndTails.interface';
import { logError } from '../../utilities/logger';
import { fetchMatchHistoryFromDB, getAllBetHistoryFromDB, getUserHistoryFromDB } from '../../utilities/db-queries';
...
export async function getAllBetHistory(req: Request, res: Response): Promise<void> {
  const userId = req.headers['user_id'] as string;
  const operatorId = req.headers['operator_id'] as string;
  const token = req.headers['token'] as string;
  ...
  try {
    const data = await getAllBetHistoryFromDB();
    res.json({
      status: true,
      message: 'Bet history fetched successfully',
      data,
