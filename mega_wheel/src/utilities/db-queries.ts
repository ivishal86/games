...
import { AllBetHistory, BetData, MatchHistoryRow, SaveBetInput, UpdateBetResultParams, UserBetHistory } from "../interface/headAndTails.interface";
...
export async function getAllBetHistoryFromDB(): Promise<AllBetHistory[]> {
  const [rows] = await pool.query<RowDataPacket[]>(`
    SELECT 
    match_id, 
      user_id,
      ...
      created_at
    FROM bets
    WHERE result_status !=? AND bet_data IS NOT NULL AND result IS NOT NULL
    ORDER BY created_at DESC
    LIMIT 25
  `, ['rollback']);
...
export async function getLastRounds(userId: string): Promise<string[]> {
  const [rows] = await pool.query<RowDataPacket[]>(`
    SELECT 
      win_amount
    FROM bets
  ...
  // Return only win_amount as string
  return rows.map((row: RowDataPacket) => String(row.win_amount));
}
