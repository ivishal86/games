...
export const createRedisClient = ():Redis => {
  console.log( process.env.REDIS_USERNAME," process.env.REDIS_USERNAME",Number(process.env.REDIS_DB),"Number(process.env.REDIS_DB)")
...
export const initializeRedis = async (): Promise<Redis> => {
  ...
  while (retries < maxRetries) {
    try {
      ...
      void logInfo('✅ REDIS CONNECTION SUCCESSFUL');
      return redisClient;
    } catch (err: any) {
      retries += 1;
      void logError(
      ...
      if (retries >= maxRetries) {
        void logError('Maximum retries reached. Could not connect to Redis.');
...
export const incrementCounter = async (key: string) => {
  if (!redisClient) await initializeRedis();
  try {
    let newValue = await redisClient!.incr(key);
    return newValue;
  } catch (error: any) {
    console.log(error.stack);
    logError(`Failed to increment counter: ${error.message}`);

  }
};

export const resetCounter = async (key: string) => {
  if (!redisClient) await initializeRedis();
  try {
    ...
    console.log(error.stack);
    logError(`Failed to reset counter: ${error.message}`)
    throw error;
  }
};