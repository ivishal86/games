...
import { logInfo, logError, logRedis, logCashout, logThirtParty, logFailedThirtParty } from '../utilities/logger';
...
export const DEBIT_TXN_TYPE = 0;
export const CREDIT_TXN_TYPE = 1;
...
export function createDebitObject(
  ...
  return {
    ...
    description: `${betAmount} debited for October Pub Round ${matchId}`,
    bet_id: `BT:${matchId}:${socket.data.userInfo.operatorId}:${userId}`,
    txn_type: DEBIT_TXN_TYPE
  };
}

export function createCreditObject(
  amount: number,
  creditTxnId: string,
  refTxnId: string,
  socket: Socket,
  ...
  return {
    ...
    user_id: decodeURIComponent(userId),
    ip: socket.handshake.address || 'unknown',
    description: `${amount.toFixed(2)} Credited for October Pub Game for Round Id ${matchId}`,
    // bet_id: `BT:${matchId}:${socket.data.userInfo.operatorId}:${userId}`,
    txn_type: CREDIT_TXN_TYPE
  };
}

export async function processWinTransaction(creditObj: CreditObj, socket: Socket, token: string, matchId: string, roundId:string): Promise<void> {
