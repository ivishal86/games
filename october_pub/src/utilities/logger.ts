...
const rollbackRetryLogger = createLogger('rollback-retry', 'rollbackRetry');
...
export async function logRollbackRetry(message: string, context: Record<string, unknown> = {}): Promise<void>  {
  rollbackRetryLogger.info({ message, ...context });
}