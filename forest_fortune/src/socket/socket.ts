...
import { DECIMALS, getGameConfig } from '../utilities/helper';
  ...
  io.on('connection', async (socket: Socket) => {
    ...
    if (!gameSettings?.game_id) {
      ...
      if (gameSettings?.data.length) {
        ...
        await setHashField(`GD:${config.GAME_NAME}`, { data: data })
      } else {
    ...
    void (async (): Promise<void> => {
      try {
        ...
        emitSocketMessage({
          ...
          data: { user_id: user.user_id, balance: Number(user.balance).toFixed(2) },
        })
