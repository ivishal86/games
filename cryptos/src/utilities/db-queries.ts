...
import { BetData, MatchHistoryRow, SaveBetInput, UpdateBetResultParams, UserBetHistory } from "../interface/cryptos";
import { RollbackUpdateInput } from "../interface";
...
//       bet_data, bet_status, bet_request, Bet_response,
//       bet_txn_id, is_declared, result, result_status,
//       result_txn_id, win_amount
//     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
//   `;
...
export async function getUserHistoryFromDB(userId: string, operatorId: string): Promise<UserBetHistory[]> {
  ...
  const formatted = rows.map((row: RowDataPacket) => {
    ...
    // Parse result (always return as array)
    try {
      parsedResult =
        typeof row.result === "string" ? JSON.parse(row.result) : row.result;
      if (!Array.isArray(parsedResult)) {
        parsedResult = [parsedResult]; // fallback: wrap non-array into array
      }
    } catch {
      parsedResult = [];
    }
