...
import { GameResponse } from '../interface/cryptos';
...
export async function postBetTxn(data: PostBetTxnData): Promise<DebitData> {
  ...
  try {
    ...
    return {status:false,msg:"Bet Cancelled by Upstream Server."}
  }
}
...
export function createDebitObject(betAmount: number, debitTxnId: string, socket: Socket, matchId: string, gameId: string, userId: string): DebitObj {
  return {
    ...
    description: `${betAmount.toFixed(DECIMAL_PLACES)} Debited for Cryptos Game for Round Id ${matchId}`,
...
export function createCreditObject(winAmount: number, creditTxnId: string, socket: Socket, matchId: string, debitTxnId: string): CreditObj {
  return {
    ...
    description: `${winAmount.toFixed(DECIMAL_PLACES)} Credited for Cryptos Game for Round Id ${matchId}`,
...
export function emitResult(socket: Socket,event:keyof typeof EVENT_TYPES, response: GameResponse): void {
