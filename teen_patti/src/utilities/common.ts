...
import { GameResponse, Card } from '../interface/teenPatti.interface';
import { getRedisClient } from './redis-connecton';
import { randomInt } from 'crypto';
import { suits, ranks } from './helper';
...
export async function postBetTxn(data: PostBetTxnData, socket:Socket): Promise<DebitData> {
  ...
  console.log(webhookData,"webhoook----------------------------")
  try {
...
export function createDebitObject(betAmount: number, debitTxnId: string, socket: Socket, matchId: string, gameId: string, userId: string): DebitObj {
  return {
    ...
    description: `${betAmount.toFixed(DECIMAL_PLACES)} Debited for Teen Patti Champion Game for Round Id ${matchId}`,
    bet_id: `BT:${matchId}:${socket.data.userInfo.operatorId}:${decodeURIComponent(socket.data.userInfo.user_id)}`,
...
export function createCreditObject(winAmount: number, creditTxnId: string, socket: Socket, matchId: string, debitTxnId: string): CreditObj {
  return {
    ...
    description: `${winAmount.toFixed(DECIMAL_PLACES)} Credited for Teen Patti Champion Game for Round Id ${matchId}`,
...
export function emitResult(io: Server,event:keyof typeof EVENT_TYPES, response: GameResponse): void {
...
export async function getUserFromApi(token: string): Promise<User> {
  try {
    ...
    throw new Error(
      error.response?.data?.msg || 'Invalid Token or session timed out',
    );
  }
}
