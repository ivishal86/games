...
import { BetData, MatchHistoryRow, SaveBetInput, UpdateBetResultParams, UserBetHistory } from "../interface/teenPatti.interface";
import { BetAttributes, RollbackUpdateInput } from "../interface";
import { logDatabase } from "./logger";
import { calculateAverageMultiplier, processHandComparison } from "./helper";
...
export async function saveBetToDB(data: SaveBetInput): Promise<void> {
  const query = `
    INSERT INTO bets (
      user_id, bet_id, match_id, operator_id, bet_amount,
...
export async function updateBetResult(params: UpdateBetResultParams, status: string): Promise<void> {
  try {
    ...
    let sql;
    if (status == 'win') {
      sql = `UPDATE bets SET ${fields.join(', ')} WHERE bet_id = ? AND bet_txn_id = ? AND user_id = ? AND match_id = ? AND result_txn_id is NULL`;
...
export async function getUserHistoryFromDB(userId: string, operatorId: string): Promise<UserBetHistory[]> {
  ...
  const formatted = rows.map((row: RowDataPacket) => {
    ...
    let dealer_total: string | number = 0;
    let player_total: string | number = 0;

    if (parsedResult?.dealerHand && parsedResult?.playerHand) {
      const totals = processHandComparison(parsedResult.playerHand, parsedResult.dealerHand);
...
export async function getLastRounds(userId: string): Promise<any[]> {
  const [rows] = await pool.query<RowDataPacket[]>(`
    SELECT 
      result
    FROM bets
  ...
  const formatted = rows
    .map((row: RowDataPacket) => {
      try {
        ...
        return parsed?.result; // only return "player" or "dealer"
      } catch {
        return null;
      }
    })
    .filter(r => r !== null); // remove any invalid entries
