...
//       } else if (result?.status === false && (result?.msg === "Bet already rollback-ed for the given txn" || result?.msg === "Transaction not found")) {
//         await deleteHashField(result.item);
//       }
//       else {
  ...
  try {
    ...
    if (values.winAmount) {
      const webHookData = {
        ...
        description: `${values.winAmount} Credited (Server Restart) for Teen Patti Champion Game for Round Id ${values.matchId}`,
        txn_type: 1,
      };
